...
static int read_line(char *buf, size_t cap) {
    ...
    while (1) {
        ...
        if (c == 3) {
            puts("^C"); buf[0] = 0;
            tcsetattr(STDIN_FILENO, TCSAFLUSH, &saved); return 0;
        }
        if (c == 127 || c == 8) {
            if (cursor) {
    ...
    while (*p && n < MAX_TOK - 1) {
        ...
        while (*p) {
            if (*p == '\'' || *p == '"') {
                ...
                else if (j + 1 < sizeof(tmp)) tmp[j++] = *p;
                p++; continue;
            }
...
static int builtin(char **a) {
    ...
    if (!strcmp(a[0], "echo")) {
        for (int i = 1; a[i]; i++) { if (i > 1) putchar(' '); fputs(a[i], stdout); }
        putchar('\n'); return 1;
    }
    ...
    if (!strcmp(a[0], "export")) {
        if (a[1]) { char *eq = strchr(a[1], '='); if (eq) { *eq = 0; setenv(a[1], eq + 1, 1); } }
        return 1;
    }
...
static int execute(char **tok, int ntok) {
    ...
    for (int i = 0; i < ntok; i++) {
        if (!strcmp(tok[i], "|")) { tok[i] = NULL; cmd[++ncmd] = &tok[i + 1]; }
    }
    ncmd++;
